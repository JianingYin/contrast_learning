...
def finetune_oasis_model(pretrained_model_path, oasis_brain_root, oasis_hipp_root, output_dir):
    ...
    def progressive_unfreeze(model, epoch):
        ...
        # 阶段1：Epoch 1-10，只解冻分类头
        if 1 <= epoch <= 10:
            print(f"Epoch {epoch}: 只解冻分类头")
            for param in model.cls_head.parameters():
                param.requires_grad = True
            
        # 阶段2：Epoch 11-30，额外解冻layer4
        elif 11 <= epoch <= 30:
            print(f"Epoch {epoch}: 解冻分类头和layer4")
            # 解冻分类头
            for param in model.cls_head.parameters():
                param.requires_grad = True
            
            # 解冻layer4（假设对应encoder.encoder.9-12）
            layer4_indices = list(range(9, 13))  # 9-12的范围
            for i in layer4_indices:
                if 0 <= i < len(model.encoder.encoder):
                    for param in model.encoder.encoder[i].parameters():
                        param.requires_grad = True
        
        # 阶段3：Epoch 31-50，再解冻layer3
        elif 31 <= epoch <= 50:
            print(f"Epoch {epoch}: 解冻分类头、layer3和layer4")
            # 解冻分类头
            for param in model.cls_head.parameters():
                param.requires_grad = True
            
            # 解冻layer4（假设对应encoder.encoder.9-12）
            layer4_indices = list(range(9, 13))
            for i in layer4_indices:
                if 0 <= i < len(model.encoder.encoder):
                    for param in model.encoder.encoder[i].parameters():
                        param.requires_grad = True
            
            # 解冻layer3（假设对应encoder.encoder.6-8）
            layer3_indices = list(range(6, 9))  # 6-8的范围
            for i in layer3_indices:
                if 0 <= i < len(model.encoder.encoder):
                    for param in model.encoder.encoder[i].parameters():
                        param.requires_grad = True
    ...
    print("\n参数冻结配置:")
    print("训练策略: 分阶段解冻")
    print("# Epoch 1-10: 只解冻分类头")
    print("# Epoch 11-30: 额外解冻layer4 (encoder.encoder.9-12)")
    print("# Epoch 31-50: 再解冻layer3 (encoder.encoder.6-8)")
    print("# BatchNorm层设置为训练模式以更新统计量，但冻结其参数")
    print("分类头已添加Dropout(p=0.3)以减少过拟合")
    
    # 初始调用progressive_unfreeze函数设置第一阶段（epoch 1）的参数冻结状态
    progressive_unfreeze(model, 1)
    ...
    # 分层学习率优化器 - 初始设置为只包含分类头参数
    # 使用Adam优化器的默认betas参数
    optimizer = torch.optim.Adam(model.cls_head.parameters(), 
                                lr=1e-4, 
    ...
    for epoch in range(config['num_epochs']):
        # 调用progressive_unfreeze函数更新当前epoch的参数冻结状态
        current_epoch = epoch + 1  # 转换为1-based的epoch索引
        progressive_unfreeze(model, current_epoch)
        
        # 创建新的优化器参数组，确保每个组都包含所有必要的优化器参数
        param_groups = []
        
        # 添加分类头参数组
        cls_params = [p for p in model.cls_head.parameters() if p.requires_grad]
        if cls_params:
            param_groups.append({
                'params': cls_params,
                'lr': 1e-4,
        ...
        # 如果epoch >= 11，添加layer4参数组
        if current_epoch >= 11:
            layer4_params = [p for i in range(9, 13) if 0 <= i < len(model.encoder.encoder) 
                           for p in model.encoder.encoder[i].parameters() if p.requires_grad]
            if layer4_params:
                param_groups.append({
        ...
        # 如果epoch >= 31，添加layer3参数组
        if current_epoch >= 31:
            layer3_params = [p for i in range(6, 9) if 0 <= i < len(model.encoder.encoder) 
                           for p in model.encoder.encoder[i].parameters() if p.requires_grad]
            if layer3_params:
                param_groups.append({
        ...
        if epoch < 10:
            ...
            phase_desc = "(解冻分类头和layer4)"
        else:
            phase_desc = "(解冻分类头、layer3和layer4)"
    ...
    # 注意：由于只训练分类器，encoder的BatchNorm参数不会被更新
    # 在实际应用中，可以考虑使用BatchNorm的track_running_stats=True来让模型适应新数据分布
...
if __name__ == "__main__":
    ...
    # 结果保存路径 - 修改为在finetune目录下创建runs文件夹
