...
def finetune_oasis_model(pretrained_model_path, oasis_brain_root, oasis_hipp_root, output_dir):
    ...
    def progressive_unfreeze(model, epoch):
        ...
        # Epoch 26-50: 额外解冻layer4（encoder.encoder.7和8）和所有BatchNorm层
        elif 26 <= epoch <= 50:
            print(f"Epoch {epoch}: 解冻分类头、layer4（encoder.encoder.7和8）和所有BatchNorm层")
            # 解冻分类头
            for param in model.cls_head.parameters():
                param.requires_grad = True
            
            # 解冻layer4相关层（encoder.encoder.7和8）
            layer4_indices = [7, 8]
    ...
    print("\n参数冻结配置:")
    print("训练策略: 分阶段解冻")
    print("# Epoch 1-25: 只解冻分类头")
    print("# Epoch 26-50: 额外解冻 layer4 (encoder.encoder.7和8)")
    ...
    optimizer = torch.optim.Adam([
        ...
        {'params': model.encoder.encoder[9:].parameters(), 'lr': 5e-5},
        {'params': model.encoder.encoder[6:9].parameters(), 'lr': 1e-5},
    ...
    print("\n注意事项：")
    print("1. 解冻整个分类头、layer3、layer4和所有BatchNorm层")
    print("2. 解冻的具体层为：cls_head所有参数，encoder.encoder.5, encoder.encoder.6 (layer3), ")
    print("   encoder.encoder.7, encoder.encoder.8 (layer4)，以及所有BatchNorm层")
    print("3. 分类头中添加了Dropout(p=0.3)以减少过拟合")
    print("4. 采样比例为15%，使用分层采样方式确保各类别样本比例")
    print("5. 所有BatchNorm层设置为训练模式，允许更新统计信息")
